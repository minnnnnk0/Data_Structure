// 우선순위 큐
// 비교하는 조건은 priority로 바꿔준다. 
// (옵셔널 체이닝으로 대비, 양쪽 Node들이 or 한 쪽이 undefined일 경우)
...
class PriorityQueue {
    ...
    #reheapDown(index) {
      if (leftIndex < this.arr.length) {
        ...
        // 왼쪽은 다 채워져있겠지만, 오른쪽 Node가 없을 수 있으니 옵셔널로 대비
        const bigger =
  ...
  // 중복값을 넣어버리면 정렬이 달라지기 때문에 최대한 중복은 피하기
  // 그리고 일반 노드들은 1,2,3,4...로 만들고 우선순위 큐는 100 (최소힙에선 -100)으로 값을 주는것을 추천
  // 우선순위는 값에 차이가 많이나도록 